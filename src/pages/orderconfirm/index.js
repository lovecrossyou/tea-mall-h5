...
const Footer = ({onClick})=>{
  return <div className={styles.confirm_footer} onClick={onClick}>
    去支付
  </div>
}

const ProductOption = ({ icon, title = "", right }) => {
  return <div className={styles.p_option}>
...
const ProductOptions = () => {
  return <div>
    <ProductOption icon={number_icon} title='购买数量' right={<Stepper
      ...
      defaultValue={3}
    />}/>
    <ProductOption icon={distribution_icon} title='配送方式' right={<div className={styles.delivery}>快递￥6.00</div>}/>
    <ProductOption icon={coupons_icon} title='优惠券' right={<div className={styles.delivery}>暂无可用优惠券</div>}/>
  </div>;
};

const ProductOptionFooter = () => {
  return <div className={styles.p_opt_footer}>
    <ProductOption icon={money_icon} title='实付款' right={<div></div>}/>
  </div>;
};
  ...
  return <div>
    ...
    <div className={styles.p_info}>
      ...
      <div className={styles.p_info_wrapper}>
        <div className={styles.p_name}>买一送一】蜜桃乌龙茶 白桃乌龙蜜
          茶三角茶包水果味茶蜜桃茶
        </div>
